...
def cansel(message):
    bot.send_message(message.chat.id, "❌ Чтобы посмотреть команды, используй - /info", reply_markup=ReplyKeyboardRemove())
...
def start_command(message):
    bot.send_message(message.chat.id, "👋 Привет! Я твой бот-менеджер проектов!\n"
        "Помогу тебе сохранить проекты, навыки и отслеживать прогресс 🚀\n"
        "Чтобы узнать команды, введи /info")
    info(message)
...
📋 /projects — Посмотреть список своих проектов
🛠️ /skills — Добавить навык к проекту
...
def link_project(message, data):
    ...
    bot.register_next_step_handler(message, lambda msg: callback_project(msg, data, statuses))
